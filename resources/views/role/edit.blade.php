<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="w-full">
                <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" action="{{ route('role.update', ['id' =>$role->id]) }}"
                    method="POST">
                    @csrf
                    <div class="mb-4">
                        ...
                        @if ($permissions->isNotEmpty())
                            @foreach ($permissions as $permission)
                                <div class="flex items-center">
                                    <input {{ $hasPermissions->contains($permission->name) ? 'checked' : '' }} name="permissions[]" type="checkbox" value="{{ $permission->name }}"
                                        id="permission-{{ $permission->id }}">
                                    <label for="permission-{{ $permission->id }}"
                                        class="block text-gray-700 px-2">{{ $permission->name }}</label>
                                </div>
                            @endforeach
                        @endif
